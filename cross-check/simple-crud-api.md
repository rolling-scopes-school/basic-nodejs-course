...
*  Приложение запускается в development-режиме при помощи `nodemon` (имеется `npm` скрипт `start:dev`, который запускает отслеживание изменений в главном  (несобранном) файле приложения) **плюс 6 баллов**
